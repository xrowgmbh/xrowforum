{section var=attributes loop=$content_attributes}
    {if and($attributes.data_type_string|eq('ezobjectrelationlist'),$object.content_class.id|eq(4))}
        {def $group = ''
             $current_user = fetch( 'user', 'current_user' )
             $admin_group_contentobject_id = 12
        }
        {foreach $current_user.groups as $key => $group}
            {set $group=fetch( 'content', 'object', hash( 'object_id', $current_user.groups[$key] ) )}
            {if $group.main_node.contentobject_id|eq($admin_group_contentobject_id)}
